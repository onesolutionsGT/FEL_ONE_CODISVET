...
namespace FEL_ONE.Forms
{
    class BatchFormA
    {
        private string XmlForm = (System.Windows.Forms.Application.StartupPath + @"\SRFS\BatchAnulacion.srf").Replace(@"\\", @"\");
        ...
        public BatchFormA()
        {
            try
            {
                ...
                if (Utils.ActivateFormIsOpen(SBO_Application, "SBOBatchA") == false)
                {
                    LoadFromXML(XmlForm);
                    oForm = SBO_Application.Forms.Item("SBOBatchA");
                    oForm.Visible = true;
                    oForm.PaneLevel = 1;
                    ...
                    oForm.Title = "One Solutions - Anulacion por Lote";
                    LlenaSeries();
                    setEvents();
                }
                else
        ...
        {
            SBO_Application
                .ItemEvent += new SAPbouiCOM._IApplicationEvents_ItemEventEventHandler(SBO_Application_ItemEvent_SBOBATCHA);
        }
        ...
        private void SBO_Application_ItemEvent_SBOBATCHA(string FormUID, ref ItemEvent pVal, out bool BubbleEvent)
        {
            BubbleEvent = true;
            try
            {
        ...
        private void LlenaSeries()
        {
            ...
            try
            {
                ...
                {
                    sql += "CALL FELONE_UTILS ('LLENASERIESA','" + user + "','','','','','','','','','') ";
                }
                else
                {
                    sql += "EXEC FELONE_UTILS 'LLENASERIESA','" + user + "','','','','','','','','','' ";
                }
        ...
        private void LlengaGrid(string Serie, bool incluirRechazadas, string del, string al)
        {
            ...
            try
            {
                if (oCompany.DbServerType == SAPbobsCOM.BoDataServerTypes.dst_HANADB)
                {
                    sql = "CALL FELONE_UTILS ('LISTADOBATCHA','" + Serie + "','" + del + "','" + al + "','','','','','','','') ";
                }
                else
                {
                    sql = "EXEC FELONE_UTILS 'LISTADOBATCHA','" + Serie + "','" + del + "','" + al + "','','','','','','','' ";
                }
                        ...
                        for (var i = 0; i <= oGrid.Rows.Count - 1; i++)
                        {
                            switch (Utils.FEL)
                            {
                                case Utils.TipoFEL.MEGAPRINT:
                                    Megaprint.EnviaDocumentoFELA(oCompany, SBO_Application, Tipo, Serie, oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(1).Name, i).ToString(), SerieName, oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(5).Name, i).ToString());
                                    break;
                                case Utils.TipoFEL.INFILE:
                                    Infile.EnviaDocumentoFELA(oCompany, SBO_Application, Tipo, Serie, oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(1).Name, i).ToString(), SerieName, oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(5).Name, i).ToString());
                                    break;
                                case Utils.TipoFEL.G4S:
                                    G4s.EnviaDocumentoFELA(oCompany, SBO_Application, Tipo, Serie, oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(1).Name, i).ToString(), SerieName, pais, oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(5).Name, i).ToString(), true);
                                    break;
                                case Utils.TipoFEL.DIGIFACT:
                                    Digifact.EnviaDocumentoFELA(oCompany, SBO_Application, Tipo, Serie, oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(1).Name, i).ToString(), SerieName, pais, oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(5).Name, i).ToString(), true);
                                    break;
                                case Utils.TipoFEL.TEKRA:
                                    Tekra.EnviaDocumentoFELA(oCompany, SBO_Application, Tipo, Serie, oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(1).Name, i).ToString(), SerieName, oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(5).Name, i).ToString(), "SI");
                                    break;
                                case Utils.TipoFEL.ECOFACTURAS:
                                    EcoFacturas.EnviaDocumentoFELA(oCompany, SBO_Application, Tipo, Serie, (oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(1).Name, i)).ToString(), SerieName, pais, (oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(5).Name, i)).ToString(), true);
                                    break;
                                case Utils.TipoFEL.GUATEFACTURAS:
                                    Guatefacturas.EnviaDocumentoFELA(oCompany, SBO_Application, Tipo, Serie, (oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(1).Name, i)).ToString(), SerieName, pais, (oGrid.DataTable.GetValue(oGrid.DataTable.Columns.Item(5).Name, i)).ToString(), true);
                                    break;
                            }
                            cont += 1;
