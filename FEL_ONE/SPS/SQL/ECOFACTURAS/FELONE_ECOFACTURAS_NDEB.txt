...
/****** Object:  StoredProcedure [dbo].[FELONE_ECOFACTURAS_NDEB]    Script Date: 8/4/2022 5:01:30 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[FELONE_ECOFACTURAS_NDEB] (@DOCENTRY INT,@DECIMAL INT)
AS
...
DECLARE @U_NUMERO_DOCUMENTO_NC VARCHAR(500)
DECLARE @U_Serie VARCHAR(500)
DECLARE @U_dte_numero VARCHAR(500)
DECLARE @U_FECHA_NC DATE
...
select 
@U_NUMERO_DOCUMENTO_NC = isnull(U_NUMERO_DOCUMENTO_NC,''), 
@U_dte_numero = isnull(U_NUMERO_DOCUMENTO,''),
@U_Serie = isnull(U_Serie_Face,''),
@U_FECHA_NC = isnull(U_FECHA_ENVIO_FACE,'')
from OINV where DocEntry = @Docentry



SET @DTEDETA=@DTEDETA+'<stdTWSNota>' 
SET @DTEDETA=@DTEDETA+'<stdTWS.stdTWSNota.stdTWSNotaIt>' 
SET @DTEDETA=@DTEDETA+'<TDFEPRegimenAntiguo>0</TDFEPRegimenAntiguo>' 
SET @DTEDETA=@DTEDETA+'<TDFEPAutorizacion>' + @U_NUMERO_DOCUMENTO_NC + '</TDFEPAutorizacion>' 
SET @DTEDETA=@DTEDETA+'<TDFEPSerie>' + @U_Serie + '</TDFEPSerie>' 
SET @DTEDETA=@DTEDETA+'<TDFEPNumero>' + @U_dte_numero + '</TDFEPNumero>' 
SET @DTEDETA=@DTEDETA+'<TDFEPFecEmision>' + CONVERT(varchar,@U_FECHA_NC,23) + '</TDFEPFecEmision>' 
SET @DTEDETA=@DTEDETA+'</stdTWS.stdTWSNota.stdTWSNotaIt>' 
SET @DTEDETA=@DTEDETA+'</stdTWSNota>'
