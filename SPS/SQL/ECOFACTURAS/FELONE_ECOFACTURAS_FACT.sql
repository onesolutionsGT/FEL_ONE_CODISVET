...
/****** Object:  StoredProcedure [dbo].[FELONE_ECOFACTURAS_FACT]    Script Date: 8/4/2022 5:00:14 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[FELONE_ECOFACTURAS_FACT] (@DOCENTRY INT,@DECIMAL INT)
AS

DECLARE @DTEENCA VARCHAR(MAX)
DECLARE @DTEDETA VARCHAR(MAX)
DECLARE @DTEDETAAUX VARCHAR(MAX)
...
DECLARE @CuentaAjena  VARCHAR(25)

--DENARIUM FECHA DESCRIPCION FACTURA
DECLARE @DescConFecha  VARCHAR(100)
DECLARE @FECHAOINV VARCHAR(25)
DECLARE @FECHAOCRD VARCHAR(25)
DECLARE @FechaDesc date
...
SELECT DISTINCT			
			...
			@Tipo = 'FACT',
			@DocNum = t0.DocNum,
			@FechaHoraEmision = convert(varchar, t0.TaxDate, 23),
			...
			@Valor5 = '',
			@Valor6 = '',
			@FECHAOINV = ISNULL(t0.U_Envia_Fecha,'N'),
			@FECHAOCRD = ISNULL(t3.U_Tipo_Facturacion,'A')
			from OINV t0 
...
IF @ICodigoUnidadGravable = 'EXE'
	BEGIN
		...
		SET @DTEENCA=@DTEENCA+ '<TrnEscCod>24</TrnEscCod>'
	END
ELSE
	BEGIN
		SET @Exento = 0
...
ELSE
	BEGIN
		SET @FechaDesc = CAST(@FechaHoraEmision as date)
		IF @FECHAOCRD = 'A'
			BEGIN
				 SET @FechaDesc = @FechaDesc
			END
		ELSE
			BEGIN
				SET @FechaDesc =  DATEADD(MM, -1, @FechaDesc)
			END
		select 
			@DescConFecha = 
				CASE
					WHEN MONTH(@FechaDesc) = 1 THEN ' Enero'+  ' de '+ CAST(YEAR(@FechaDesc) as varchar(100))
					WHEN MONTH(@FechaDesc) = 2 THEN ' Febrero'+  ' de '+ CAST(YEAR(@FechaDesc) as varchar(100))
					WHEN MONTH(@FechaDesc) = 3 THEN ' Marzo'+  ' de '+ CAST(YEAR(@FechaDesc) as varchar(100))
					WHEN MONTH(@FechaDesc) = 4 THEN ' Abril'+  ' de '+ CAST(YEAR(@FechaDesc) as varchar(100))
					WHEN MONTH(@FechaDesc) = 5 THEN ' Mayo'+  ' de '+ CAST(YEAR(@FechaDesc) as varchar(100))
					WHEN MONTH(@FechaDesc) = 6 THEN ' Junio'+  ' de '+ CAST(YEAR(@FechaDesc) as varchar(100))
					WHEN MONTH(@FechaDesc) = 7 THEN ' Julio'+  ' de '+ CAST(YEAR(@FechaDesc) as varchar(100))
					WHEN MONTH(@FechaDesc) = 8 THEN ' Agosto'+  ' de '+ CAST(YEAR(@FechaDesc) as varchar(100))
					WHEN MONTH(@FechaDesc) = 9 THEN ' Septiembre'+  ' de '+ CAST(YEAR(@FechaDesc) as varchar(100))
					WHEN MONTH(@FechaDesc) = 10 THEN ' Octubre'+  ' de '+ CAST(YEAR(@FechaDesc) as varchar(100))
					WHEN MONTH(@FechaDesc) = 11 THEN ' Noviembre'+  ' de '+ CAST(YEAR(@FechaDesc) as varchar(100))
					WHEN MONTH(@FechaDesc) = 12 THEN ' Diciembre'+  ' de '+ CAST(YEAR(@FechaDesc) as varchar(100))
					ELSE 'N/A'
END;	
	END
...
SET @DTEDETA= ''
SET @DTEDETAAUX = ''
SET @DTEDETAAUX = @DTEDETAAUX + '<stdTWSCA1>'
SET @CuentaAjena = 'NO'
	...
	group by a."ItemCode",a."LineTotal",a."VatSum",a."Currency",a."TotalFrgn",a."VatSumFrgn",a."Quantity",a."Dscription",a."TaxCode",a."PriceBefDi","INMPrice", a.U_Tipo;
		...
		IF @BienOServicio <> 'J'
			BEGIN
				...
				SET @DTEDETA=@DTEDETA+ '<TrnArtNom>' + @Descripcion + @DescConFecha  +'</TrnArtNom>' 
		...
		ELSE		
			BEGIN
				SET @CuentaAjena = 'SI'
				SET @DTEDETAAUX=@DTEDETAAUX+ '<stdTWS.stdTWSCA1.stdTWSCA1It>'
				SET @DTEDETAAUX=@DTEDETAAUX+ '<Columna1>'+@Descripcion+ @DescConFecha  +'</Columna1>'
				SET @DTEDETAAUX=@DTEDETAAUX+ '<Columna2>'+@IDReceptor+'</Columna2>'
				SET @DTEDETAAUX=@DTEDETAAUX+ '<Columna3>'+@Descripcion+'</Columna3>'
				SET @DTEDETAAUX=@DTEDETAAUX+ '<Columna4></Columna4>'
				SET @DTEDETAAUX=@DTEDETAAUX+ '<Columna5>'+convert(varchar,@PrecioUnitario * @Cantidad)+'</Columna5>'
				SET @DTEDETAAUX=@DTEDETAAUX+ '<Columna6></Columna6>'
				SET @DTEDETAAUX=@DTEDETAAUX+ '<Columna7></Columna7>'
				SET @DTEDETAAUX=@DTEDETAAUX+ '<Columna8></Columna8>'
				SET @DTEDETAAUX=@DTEDETAAUX + '</stdTWS.stdTWSCA1.stdTWSCA1It>'
			END
...
IF @CuentaAjena = 'SI'
	BEGIN
		SET @DTEDETA=@DTEDETA + @DTEDETAAUX
	END
